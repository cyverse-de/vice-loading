...
export let fetchUpdates = () => {
    return (dispatch) => {
        ...
        return axios
            .get(`/api/jobs/status-updates?url=${searchParams.get("url")}`)
            .then((response) =>
                response.data.job_status_updates.forEach((i) =>
                    dispatch(addUpdate(i))
                )
            )
...
export const checkURLReady = () => {
    return (dispatch) => {
        let searchParams = new URLSearchParams(window.location.search);
        return axios
            .get(`/api/url-ready?url=${searchParams.get("url")}`)
            .then((response) => dispatch(setReady(response.data.ready)))
            .catch(function(error) {
                console.log("error from server: ", error.message);
            });
    };
            ...
            return Object.assign({}, state, {
                entities: Object.assign({}, state.entities, {
                    updates: Object.assign({}, state.entities.updates, {
                        [Object.keys(state.entities.updates).length]:
                            action.payload,
                    }),
                }),
            });
